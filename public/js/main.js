...
var calcBPM = function(fname) {
  $.ajax({
    ...
    success: function(data) {
      ...
      if(data) {
        BPM = Number(data.bpm);
        console.log("Remembered bpm " + BPM);
      } else {
...
(function() {
  ...
  a.bind = function() {
    ...
    $('.buffer').on(click, function() {
      window.location.href = 'http://preziotte.com'
    });
    ...
    $('.icon-github2').on(click, function() {
      window.open('http://github.com/preziotte/party-mode', '_blank');
    });
    ...
    $('#slider').on('input change', function() {
      analyser.smoothingTimeConstant = 1 - (this.value / 100);
    });
  ...
  r.equal = function() {
    ...
    squares.enter()
      ...
      .attr("transform", function(d, i) {
        return "translate(" + ((i * w) + marginX) + "," + ((State.height / 2) - (y(d) / 2)) + ")";
      })
  ...
  r.equal_thumb = function() {
    ...
    squares.enter()
      .append("g")
      .attr("transform", function(d, i) {
        return "translate(" + ((i * w) + marginX) + "," + ((height / 2) - (y(d) / 2)) + ")";
      })
      .attr("opacity", function(d) {
        return opacity(d);
      })
  ...
  h.handleDrop = function(e) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState == 4) {
        ...
        window.history.pushState(fname, 'ConductMe', "/?audio="+fname);
        calcBPM(fname);
